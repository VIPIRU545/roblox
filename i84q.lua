...
c.__index=c end _G.BrickColor=c return b end function a.c()local b:(x:number,y:
number,z:number)->Vector3__DARKLUA_TYPE_b local c={}do function c.dot(d,e:
...
d.z return b(e,f,g)end}end function a.d()local b=a.load'c'local c,d,e,f=math.abs
,math.sqrt,math.sin,math.cos local g,h,i=math.acos,math.atan2,math.clamp
...
}, {k.r20}, {k.r21}, {k.r22})`end end return table.freeze(j)end function a.e()
local b,c,d,e=math.abs,math.ceil,math.floor,math.sign local f,g=math.max,math.
min local h={}do local function constructor(i:number,j:number)assert('number'==
type(i),`Vector2.new: expected a number for x, got {type(i)}`)assert('number'==
type(j),`Vector2.new: expected a number for y, got {type(j)}`)return
setmetatable({x=i,y=j},h)end h.new=constructor function h.abs(i)return
constructor(b(i.x),b(i.y))end function h.cross(i,j)assert('table'==type(j)and
getmetatable(j)=='Vector2',`Vector2.cross: expected a Vector2, got {type(j)}`)
...
table.freeze(h)end end _G.Signal=a.load'a'_G.Color3=a.load'b'_G.Vector3=a.load
'c'_G.CFrame=a.load'd'_G.Vector2=a.load'e'
